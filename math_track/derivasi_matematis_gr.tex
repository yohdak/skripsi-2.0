...
\newcommand{\dual}{\bm{\vartheta}}
\newcommand{\Lie}{\mathcal{L}}
\newcommand{\Tr}{\operatorname{Tr}}

\title{Jalur Derivasi Matematis Relativitas Umum\\
\large Dari Kovektor hingga Presesi Lense--Thirring}
...
\chapter{Vektor, Kovektor, dan Tensor}
...
\section{Ruang dual dan kovektor}

Ruang dual $T_p^*\mathcal{M}$ adalah ruang semua pemetaan linear
$\omega:T_p\mathcal{M}\to\mathbb{R}$. Basis dual $\{\dual^\mu\}$ didefinisikan oleh
\[
 \dual^\mu(\e_\nu)=\delta^\mu{}_\nu.
\]
Dalam basis koordinat, $\dual^\mu=\dd x^\mu$. Kovektor diekspansi sebagai
\[
 \omega=\omega_\mu\dd x^\mu.
\]
Pasangan alami kovektor dan vektor menghasilkan skalar:
\[
 \omega(V)=\omega_\mu V^\nu\dd x^\mu(\pd_\nu)
 =\omega_\mu V^\nu\delta^\mu{}_\nu
 =\omega_\mu V^\mu.
\]
Karena hasilnya skalar, komponen kovektor harus bertransformasi sebagai
\[
 \omega_{\mu'}=\frac{\pd x^\mu}{\pd x^{\mu'}}\omega_\mu.
\]

Gradien fungsi skalar secara alami merupakan kovektor:
\[
 \dd f=\pd_\mu f\,\dd x^\mu,
 \qquad (\dd f)(V)=V^\mu\pd_\mu f.
\]
Jadi $\pd_\mu f$ membawa indeks bawah bukan karena konvensi visual, tetapi karena
ia menerima sebuah vektor dan menghasilkan skalar.

\section{Produk tensor dan tipe tensor}

Produk tensor vektor $V$ dan kovektor $\omega$ ditentukan oleh bilinearitas:
\[
 V\otimes\omega
 =V^\mu\omega_\nu\,\e_\mu\otimes\dd x^\nu.
\]
...
\[
 g^{\mu\alpha}g_{\alpha\nu}=\delta^\mu{}_\nu.
\]
Metrik membentuk isomorfisme antara vektor dan kovektor:
\begin{align}
 V_\mu&=g_{\mu\nu}V^\nu,\\
 V^\mu&=g^{\mu\nu}V_\nu.
\end{align}
...
\[
 \nabla_\mu\e_\nu=\Gamma^\rho{}_{\mu\nu}\e_\rho.
\]
Turunan kovarian komponen vektor kemudian menjadi
\[
 \boxed{\nabla_\mu V^\nu=\pd_\mu V^\nu+\Gamma^\nu{}_{\mu\lambda}V^\lambda.}
\]
Untuk menjaga aturan Leibniz pada skalar $\omega_\nu V^\nu$,
\begin{align}
 \pd_\mu(\omega_\nu V^\nu)
 &=(\nabla_\mu\omega_\nu)V^\nu+\omega_\nu\nabla_\mu V^\nu,
\end{align}
...
Tensor Ricci diperoleh dengan mengontraksi indeks pertama dan ketiga:
\[
 \boxed{R_{\sigma\nu}=R^\rho{}_{\sigma\rho\nu}}.
\]
...
$(-,+,+,+)$ adalah
\begin{align}
 \dd s^2={}&-\left(1-\frac{2mr}{\Sigma}\right)(\dd x^0)^2
 -\frac{4mar\sin^2\theta}{\Sigma}\,\dd x^0\dd\phi
...
\[
 g_{0\phi}=-\frac{2mar\sin^2\theta}{\Sigma}.
\]
...
Untuk $a\to0$,
\[
 \Sigma\to r^2,\qquad\Delta\to r(r-2m),\qquad g_{0\phi}\to0,
\]
...
\begin{enumerate}[label=\arabic*.]
 \item Buktikan bahwa $\omega_\mu V^\mu$ invarian di bawah perubahan koordinat.
...
\begin{align*}
 \text{kovektor}&:\quad \omega(V)=\omega_\mu V^\mu,\\
